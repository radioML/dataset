...
class source_alphabet(gr.hier_block2):
    def __init__(self, dtype="type_discrete", limit=10000):
        if(dtype == "type_discrete"):
            print "discrete"
            ...
            self.connect(self.src,self.convert2,self.convert)
...
if __name__ == "__main__":
    ...
    tb = gr.top_block()
    src = source_alphabet("type_discrete", 1000)
    snk = blocks.vector_sink_b()
    tb.run()

    # Test continuous source
    tb = gr.top_block()
    src = source_alphabet("type_continuous", 1000)
